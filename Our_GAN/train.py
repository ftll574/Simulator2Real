...
##--- create probabilty array for DiffAug ---#
def exponential_decay(a, b, N):
    return a * (1-b) ** np.arange(N)
p_decay = exponential_decay(1, 0.005, opt.num_epochs)
...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        loss_G, losses_G_list = model(image, label, "losses_G", losses_computer, DA_p=[p_decay[epoch],1-p_decay[epoch]])
        ...
        loss_D, losses_D_list = model(image, label,"losses_D", losses_computer, DA_p=[p_decay[epoch],1-p_decay[epoch]])
