...
#--- Semantic segmentation based discriminator ---
#
#   From OASIS discriminator
#
#   Input : fake image / real image
#   Output: semantic segmentation (N real class / 1 fake class) 
...
class OASIS_Discriminator(nn.Module):
    def __init__(self, opt):
        ...
        output_channel = opt.semantic_nc + 1  #+1# for N+1 loss
        self.channels = [3, 64, 128, 256, 256, 512, 512]
...
# The residual block of discriminator
# Hyperparameters : 
#       fin : input tensor channel
#       fout : output tensor channel
#       opt : option
#       up_or_down : up sample / down sample
#       frist : Ture = is frist layer
# Input : tensor
# Output : tensor
    ...
    def __init__(self, fin, fout, opt, up_or_down, first=False):
        ...
        if first:
            ...
            if self.up_or_down > 0:
                self.conv1 = nn.Sequential(nn.LeakyReLU(0.2, True), nn.Upsample(scale_factor=2), norm_layer(nn.Conv2d(fin, fmiddle, 3, 1, 1)))
            else:
                self.conv1 = nn.Sequential(nn.LeakyReLU(0.2), norm_layer(nn.Conv2d(fin, fmiddle, 3, 1, 1)))
        self.conv2 = nn.Sequential(nn.LeakyReLU(0.2, True), norm_layer(nn.Conv2d(fmiddle, fout, 3, 1, 1)))
