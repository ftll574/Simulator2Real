...
class losses_saver():
    ...
    def __call__(self, iters, losses):
        ...
        if iters % self.freq_smooth_loss == self.freq_smooth_loss-1:
        ...
        if iters % self.freq_save_loss == self.freq_save_loss-1:
            self.plot_losses()
...
class image_saver():
    ...
    def visualize_batch(self, model, image, label, cur_iter):
        self.save_images(label[:,:-1,:,:], "label", cur_iter, is_label=True)
